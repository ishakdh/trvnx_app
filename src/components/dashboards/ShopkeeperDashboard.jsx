...
const ShopkeeperDashboard = ({ user, onLogout }) => {
    ...
    const fetchDevices = async () => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/devices/shop/${user.id || user._id}`, {
    ...
    const fetchGateways = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/settings`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('trvnx_token')}` } });
            if (res.ok) {
                const data = await res.json();
    ...
    const triggerReceivePayment = () => {
        ...
        const executePayment = async () => {
            try {
                const res = await fetch(`${import.meta.env.VITE_API_URL}/devices/collect-emi`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('trvnx_token')}` },
    ...
    const handleExtension = async () => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/devices/extend-due-date`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('trvnx_token')}` },
