...
const DistributorDashboard = ({ user, onLogout }) => {
    ...
    const fetchMyNetwork = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/auth/operators`, {
    ...
    const fetchExtendedData = async () => {
        try {
            const devRes = await fetch(`${import.meta.env.VITE_API_URL}/devices/all`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('trvnx_token')}` } });
            if (devRes.ok) {
    ...
    // 🚀 FIXED: Auto-Generate Target Name & Date Range
    useEffect(() => {
        ...
        // 🚀 THE FIX: Wrapping setTargetForm in setTimeout makes it asynchronous!
        // This completely eliminates the "cascading renders" ESLint error.
    ...
    const processShopApproval = async (shopId, action) => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/admin/approve-shopkeeper`, {
                method: 'POST',
    ...
    const handleRejectSrPayout = async () => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/transactions/reject-payout`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('trvnx_token')}` },
                body: JSON.stringify({
    ...
    const handleCreateSR = async (e) => {
        ...
        const payload = {
            ...
            father_name: srFormData.fatherName, motherName: srFormData.motherName,
